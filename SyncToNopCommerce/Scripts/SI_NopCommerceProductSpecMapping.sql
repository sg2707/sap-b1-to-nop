create procedure [dbo].[SI_NopCommerceProductSpecMapping] @ProductSku nvarchar(100)
 as
 begin
select T0.U_ControlType[control_type],T0.U_Description [name], T1.U_EntityID[attribute_id],
	case when T0.U_ControlType = 2 then T4.U_Description else T0.U_Description  end as [option_name],T1.U_Value [custom_text] from
[@SI_ATTRIBUTE] T0
...
where T1.U_EntityID=@ProductSku 
